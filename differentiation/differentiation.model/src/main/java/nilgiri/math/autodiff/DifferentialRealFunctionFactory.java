...
public class DifferentialRealFunctionFactory<X extends RealNumber<X>> {
	...
	public DifferentialFunction<X> acos(final DifferentialFunction<X> arg) {
		return new AbstractUnaryFunction<X>(arg) {
			@Override
			public X getValue() {
				return DifferentialRealFunctionFactory.this.m_factory
					.acos(this.arg().getValue());
			}

			@Override
			public String toString() {
				return String.format("acos(%s)", this.arg());
			}

			@Override
			public DifferentialFunction<X> diff(final Variable<X> variable) {
				// TODO implement the derivative of acos
				throw new UnsupportedOperationException("#diff()");
			}
		};
	}

	public DifferentialFunction<X> asin(final DifferentialFunction<X> arg) {
		return new AbstractUnaryFunction<X>(arg) {
			@Override
			public X getValue() {
				return DifferentialRealFunctionFactory.this.m_factory
					.asin(this.arg().getValue());
			}

			@Override
			public String toString() {
				return String.format("asin(%s)", this.arg());
			}

			@Override
			public DifferentialFunction<X> diff(final Variable<X> variable) {
				// TODO implement the derivative of asin
				throw new UnsupportedOperationException("#diff()");
			}
		};
	}
	...
	public DifferentialFunction<X> atan(final DifferentialFunction<X> arg) {
		return new AbstractUnaryFunction<X>(arg) {
			@Override
			public X getValue() {
				return DifferentialRealFunctionFactory.this.m_factory
					.atan(this.arg().getValue());
			}

			@Override
			public String toString() {
				return String.format("atan(%s)", this.arg());
			}

			@Override
			public DifferentialFunction<X> diff(final Variable<X> variable) {
				// TODO implement the derivative of atan
				throw new UnsupportedOperationException("#diff()");
			}
		};
	}
